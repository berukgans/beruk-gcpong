...
RegisterCommand("gcphonecok", function(source, args, rawCommand)
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    ...
    if ShowNumberNotification == true then
      text = 'Pesan baru dari '.. message.transmitter
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = 'Pesan baru dari '.. contact.display
          break
        end
      end
    end
    ...
    exports['mythic_notify']:SendAlert('inform', text, 10000)
